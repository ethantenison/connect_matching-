...
g <- ggplot(buckets, aes(x = reorder(value, n), y = n)) +
  ...
  labs(title = "Project buckets",
       y = "Number",
       fill = "Buckets",
       caption = "**Several projects had two or three categories") 
...
stu$alaska_native[is.na(stu$alaska_native)] <- ""
stu$asian[is.na(stu$asian)] <- ""
stu$black_or_african_american[is.na(stu$black_or_african_american)] <- ""
stu$hispanic_or_latino[is.na(stu$hispanic_or_latino)] <- ""
stu$native_hawaiian[is.na(stu$native_hawaiian)] <- ""
stu$other_pacific_islander[is.na(stu$other_pacific_islander)] <- ""
stu$white_or_caucasian[is.na(stu$white_or_caucasian)] <- ""
stu$i_prefer_to_specify[is.na(stu$i_prefer_to_specify)] <- ""
