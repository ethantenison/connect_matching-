...
org <- read_csv("data/raw/CONNECT_organization_spring2022.csv") |> 
  ...
  filter(which_connect_program_is_this_project_affiliated_with == "RGK Center") # Change this depending on school
...
buckets <- org |>
  ...
  mutate(
    ...
    n = case_when(
      ...
      bucket == "Data Interpretation & Analysis" ~ n + 2,
      TRUE ~ n
    ),
    bucket = case_when(
  ...
  arrange(n) |> 
  filter(bucket != "Data Collection & Tool Development,Data Interpretation & Analysis")
...
stu_geo <- read_csv("data/raw/CONNECT_rgk_students_spring2022.csv") |> 
  filter(!row_number() %in% c(2)) |> 
  janitor::row_to_names(1) |> 
  clean_names() |> 
...
stu4 <- stu4 |> 
  filter(name != "Srujan Rode")

org2 <- org2 |> 
  filter(organization != "Austin Asian Impact")


logic_modeling <- stu4 |> 
  filter(logic_modeling > 3)
