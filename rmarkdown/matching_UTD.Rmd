...
There were `r length(org$organization)` projects, but some of them involve multiple buckets. All but one project entail some form of measurement strategy. 
...
stu <- stu |>
  clean_names() |>
  filter(progress == 100) |>
  select(first_name, 56:81)

names(stu) <-
  gsub(pattern = "please_rate_your_experience_in_the_following_skills_1_not_experienced_and_5_extremely_experienced_",
       replacement = "",
       x = names(stu))
...
stu <- stu |> 
  ...
  select(
    first_name,
    last_name,
    do_you_have_other_relevant_skills_that_may_be_helpful_for_us_to_know_about_i_e_other_languages_spoken_list_them_here_note_please_separate_each_skill_with_a_comma,
...
names(stu) <- gsub(pattern = "please_rate_your_experience_in_the_following_skills_1_not_experienced_and_5_extremely_experienced_",
                  replacement = "",
                  x = names(stu))
...
stu <- stu |> 
   mutate(across(everything(), sub_text_noexp)) |> 
   mutate(across(everything(), sub_text_exp)) |> 
  #removing na from question that didn't have a force response
...
stu <- stu |> 
  mutate(across(7:34, as.numeric)) |> 
  mutate(name = paste0(first_name," ", last_name)) |> 
  select(name, time_commitment, 3:34) |> 
  #arrange dataframe by last name
  arrange(str_extract(name,'\\s.*$'))


#For table 
stu_table <- stu |> 
  select(-c(relevant_skills))
...
The following table includes students' responses to open ended questions. It's impossible to list every single skill that might be relevant for a project on the survey (although we have tried), so the relevant skills question is attempt to fill that gap.
...
stu1 <- stu[1, ] |>
  pivot_longer(7:33, names_to = "variable", values_to = "student_value")
