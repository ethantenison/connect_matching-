...
org <- read_csv("data/raw/CONNECT_organization_spring2022.csv") |> 
  ...
  filter(which_connect_program_is_this_project_affiliated_with == "UT-Dallas") # Change this depending on school

#write.csv(org, "data/processed/utd_spring_2022.csv", row.names = FALSE)
...
buckets <- org |>
  select(
    organization,
    identify_the_categories_your_project_falls_under_check_all_that_apply
  ) |>
  rename(bucket = identify_the_categories_your_project_falls_under_check_all_that_apply) |>
  group_by(bucket) |>
  count() |>
  ungroup() |> 
  add_row(bucket = "Research", n = 1) |> 
  add_row(bucket = "Data Interpretation & Analysis", n = 1) |>
  mutate(
    n = as.numeric(n),
    n = case_when(
      bucket == "Data Collection & Tool Development" ~ n + 2,
      bucket == "Measurement Strategy" ~ n + 1,
      TRUE ~ n
    ),
    bucket = case_when(
  ...
  arrange(n) |> 
  filter(bucket != "Measurement Strategy,Data Collection & Tool Development",
           bucket != "Research,Data Collection & Tool Development,Data Interpretation & Analysis")
...
g <- ggplot(buckets, aes(x = reorder(bucket, n), y = n)) +
  ...
  labs(title = "CONNECT Spring 2022 Project buckets",
       y = "Number",
       fill = "Buckets",
       caption = "**Two projects were double counted") 
...
stu <- read_csv("data/raw/CONNECT - Student Form (Spring 2022)_UTD.csv") |> 
  select(-c(1:17)) |> 
  filter(!row_number() %in% c(2)) |> 
  janitor::row_to_names(1) |> 
  clean_names() |> 
  rename(pronouns = preferred_pronouns_selected_choice,
         ...
         relevant_skills = do_you_have_other_relevant_skills_that_may_be_helpful_for_us_to_know_about_i_e_other_languages_spoken_list_them_here_note_please_separate_each_skill_with_a_comma,
         ...
         schools = which_epps_program_s_are_you_affiliated_with,
  ...
  rename_with(~ gsub('please_rate_your_experience_in_the_following_skills_1_not_experienced_and_5_extremely_experienced_', '', .x))
  
# For student profiles
#write.csv(stu, "data/processed/utd_students_spring_2022.csv", row.names = FALSE)
...
stu4 <- stu |> 
  ...
  mutate(across(6:34,~gsub("1 - Not relevant", "1", .)),
         across(6:34,~gsub("5 - Extremely relevant", "5", .)),
         across(6:34, as.numeric),
         name = paste0(first_name," ", last_name),
         across(6:34, ~replace_na(.x, 1))) |> 
  filter(name != "Ishdeep Kaur",
         name != "Azharul Islam",
         name != "Charles Bitzane",
         name != "Anuolu Balogun")

stumas <- stu4

stu4 <- stu4 |> 
  select(name, time_commitment, 4:36) |>  
  arrange(str_extract(name,'\\s.*$')) |> 
  mutate(remote = as.character(remote),
    remote = case_when(
    remote == "1" ~ "Yes",
    TRUE ~ remote
  ))
...
stu_table2 <- stumas |> 
  select(name, relevant_skills, target_population, is_there_any_additional_information_about_your_availability_that_we_should_know) |>
  rename(additional_information = is_there_any_additional_information_about_your_availability_that_we_should_know) |> 
...
stu2 <- stu |>
  mutate(across(30:55,  ~ gsub("1 - Not relevant", "1", .)),
         across(30:55,  ~ gsub("5 - Extremely relevant", "5", .)),
         across(30:55, as.numeric)) |> 
select(first_name, 30:55)
...
g <- ggplot(stu3, aes(x = reorder(Skill, n), y = n, fill = n)) +
  ...
  labs(title = "CONNECT Spring 2022 Student Skills",
...
stu1 <- stu4[1, ] |>
  pivot_longer(6:32, names_to = "variable", values_to = "student_value")
...
row_names <- org2$organization

uS <- matrix(nrow = length(org2$project_name),
             ncol = length(stu4$name),
             dimnames = list(row_names, column_names))

for (i in 1:length(org2$project_name)) {
...
row_names <- stu4$name
column_names <- org2$project_name

uO <- matrix(nrow = length(stu4$name),
             ncol = length(org2$project_name),
             dimnames = list(row_names, column_names))
...
<img src="C:/Users/tenis/Desktop/Data_Projects/connect_matching/reports/ut-dallas.png" alt="University of Texas at Dallas">
