...
#library(pdftools)   #converting pdfs this was having problems on mac 
...
```{r org_location, eval=FALSE, include=FALSE}
...
```{r student_location, eval=FALSE, include=FALSE}
...
```{r model_prep, message=FALSE, warning=FALSE, eval=FALSE}
...
```{r ml, message=FALSE, warning=FALSE, eval = FALSE}
...
```
Testing out algorithm using test dataset
...
### Assigning Tech Rating

```{r modifying_data, eval=FALSE}
...
```{r resumes, eval=FALSE}

# One student's resume did not convert. Avni. It doesn't look like we have any project
#for him though 
rs_raw <- convertpdf2txt("data/raw/2022_summer_resumes/")
rs <- as.data.frame(rs_raw) |> 
  rownames_to_column("id") |> 
  rename(resume = "rs_raw") |> 
  mutate(id = gsub("data/raw/2022_summer_resumes/", "", id),
         id = gsub("~.*", "", id))
...
res_pred_test <- predict(ra_fit, new_data = rs)
...
### Altering Utility Scores


```{r, eval=FALSE}

stu_id <- stu |> 
  select(first_name, last_name, response_id) |> 
  mutate(name = paste0(first_name," ", last_name)) |> 
  left_join(rs_pred, by = c("response_id" = "id")) |> 
  select(name, .pred_class) |> 
  rename(technical_rating = .pred_class)


org_rating <- org_nlp$technical

uo_nlp <- as.data.frame(uO) |> 
  rownames_to_column("name") |> 
  left_join(stu_id, by = "name") |> 
  select(-name)

#Downweighting  mismatched resumes
for(j in 1:nrow(uo_nlp)) {
  for (i in 1:(ncol(uo_nlp) - 1)) {
    if (org_rating[i] == "More Technical" & uo_nlp[j,18] == "Less Technical") {
      std <- sd(uo_nlp[,i])
      uo_nlp[j, i] <- uo_nlp[j,i]-std/2
    }
  }
}
...
uo_names <- as.data.frame(uO) |> 
  rownames_to_column("name") |> 
  left_join(stu_id, by = "name") |> 
  select(name)

#returning matrix to prior form
uo_nlp <- uo_nlp |> 
  select(-c(technical_rating)) |> 
  bind_cols(uo_names) |> 
  column_to_rownames(var="name")
  
uo_nlp <- as.matrix.data.frame(uo_nlp)


```


```{r nlp_match,message=FALSE, warning=FALSE, layout="l-body-outset", eval=FALSE}

#Matching Algorithm 
matching = galeShapley.marriageMarket(uS, uo_nlp)

#Creating a list of top scorers
uO2 <- as.data.frame(uo_nlp)
org_options <- tibble::rownames_to_column(uO2, "Student") 
tops <- list()
