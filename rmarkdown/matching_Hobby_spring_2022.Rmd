...
description: |
  University of Houston, Spring 2022 
author:
  - name: Ethan Tenison 
...
org <- read_csv("data/raw/CONNECT_organization_spring2022.csv") |> 
  ...
  filter(which_connect_program_is_this_project_affiliated_with == "Hobby School") # Change this depending on school
...
buckets <- org |>
  ...
  count() |>
  ungroup() |> 
  add_row(bucket = "Measurement Strategy", n = 2) |>
  mutate(
    n = as.numeric(n),
    ### Just for RGK THIS ROUND because of duplicates 
    n = case_when(
...
stu <- read_csv("data/raw/CONNECT Student (Spring 2022) - UH.csv") |> 
  select(-c(1:17)) |> 
  filter(!row_number() %in% c(2)) |> 
  janitor::row_to_names(1) |> 
  clean_names() |> 
  rename(pronouns = preferred_pronouns_selected_choice,
         ...
         schools = which_uh_schools_colleges_are_you_affiliated_with_check_all_that_apply_selected_choice,
         schools_other = which_uh_schools_colleges_are_you_affiliated_with_check_all_that_apply_other_please_specify_text,
  ...
  rename_with(~ gsub('please_rate_your_experience_on_the_following_skills_note_1_not_experienced_and_5_extremely_experienced_', '', .x))
...
stu4 <- stu |> 
  ...
  mutate(
        across(9:33,~gsub("1-Not Experienced", "1", .)),
         across(9:33,~gsub("5-Extremely Experienced", "5", .)),
         across(9:33, as.numeric),
...
stu4 <- stu4 |> 
  ...
  mutate(nonprofit_experience = as.numeric(nonprofit_experience),
...
stu_geo <- read_csv("data/raw/CONNECT Student (Spring 2022) - UH.csv") |> 
  filter(!row_number() %in% c(2)) |> 
  janitor::row_to_names(1) |> 
  clean_names() |> 
...
stu3 <- stu2 |>
  mutate(
    across(2:26,~gsub("1-Not Experienced", "1", .)),
         across(2:26,~gsub("5-Extremely Experienced", "5", .)),
         across(2:26, as.numeric)
  ) |> 
...
<img src="C:/Users/tenis/Desktop/Data_Projects/connect_matching/reports/hobby_logo.png" alt="University of Houston">
