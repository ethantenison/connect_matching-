...
description: |
  University of Houston - Hobby School of Public Affairs, Fall 2021 
author:
  - name: Ethan Tenison 
...
stu_raw <- fetch_survey(surveyID = surveys$id[18], force_request = TRUE)
...
stu <- stu_raw |> 
  ...
  clean_names() |>
  filter(progress == 100) |>
  select(first_name, 41:65)

#Removing characters from skill responses
sub_text_noexp <- function(column) {
  gsub(pattern = "-Not Experienced", 
       replacement = "",
       x = column)
}

sub_text_exp <- function(column) {
  gsub(pattern = "-Extremely Experienced",
       replacement = "",
       x = column)
}

 stu <- stu |> 
   mutate(across(everything(), sub_text_noexp)) |> 
   mutate(across(everything(), sub_text_exp)) |> 
   mutate(across(2:26, as.numeric))
   
   
  

names(stu) <-
  gsub(pattern = "please_rate_your_experience_on_the_following_skills_note_1_not_experienced_and_5_extremely_experienced_",
       replacement = "",
       x = names(stu))
...
```
# Student Colleges 
```{r student_colleges, message=FALSE, warning=FALSE}

colleges <- stu_raw |> 
  janitor::row_to_names(1) |> 
  clean_names() |>
  filter(progress == 100) |> 
  select(first_name, which_uh_schools_colleges_are_you_affiliated_with_check_all_that_apply_selected_choice) |> 
  rename(Colleges = which_uh_schools_colleges_are_you_affiliated_with_check_all_that_apply_selected_choice) |> 
  group_by(Colleges) |> 
  count() |> 
  ungroup() |> 
  add_row(Colleges = "College of Liberal Arts & Social Sciences", n = 2) |> 
  add_row(Colleges = "C.T. Bauer College of Business", n = 1) |> 
  mutate(n = case_when(
    Colleges == "Hobby School of Public Affairs" ~ 9,
    Colleges == "Graduate College of Social Work" ~ 29,
    Colleges == "College of Technology" ~ 18, 
    TRUE ~ n
  )) |> 
  filter(Colleges != c("C.T. Bauer College of Business,Graduate College of Social Work",
            "Hobby School of Public Affairs,College of Liberal Arts & Social Sciences",
            "Other (please specify)"))

theme_set(theme_classic())

g <- ggplot(colleges, aes(x = reorder(Colleges, n), y = n)) +
...
names(stu) <- gsub(pattern = "please_rate_your_experience_on_the_following_skills_note_1_not_experienced_and_5_extremely_experienced_",
                  replacement = "",
                  x = names(stu))
...
stu <- stu |> 
  ...
  mutate(across(everything(), sub_text_noexp)) |> 
  mutate(across(everything(), sub_text_exp)) |> 
  select(name, time_commitment, 4:34) |>  
  arrange(str_extract(name,'\\s.*$')) |> 
  mutate(nonprofit_experience = replace_na(nonprofit_experience, 1),
         relevant_skills = replace_na(relevant_skills, "")) |> 
  mutate(across(6:31, as.numeric),
         time_commitment = as.numeric(time_commitment)) |> 
  rename(microsoft_office_suite = microsoft_office)


#For table 
stu_table <- stu |> 
  select(-relevant_skills)
