...
### Project Deliverables Word Cloud 
...
```{r project_buckets, fig.width=8, fig.height=4}
buckets <- org |>
...
```{r student_skills, fig.width=8, fig.height=7}
...
The data from Qualtrics can be messy. Many of the variables names default to the question number. In order to make the variable names more descriptive, the actual question description is substituted. This causes the variable names to be rather long. To reduce the length of variable names, many of them have been renamed, and the `stringr` package was used to remove repetitive words. In addition, all of the variables used by the algorithm were converted to numeric values to ensure everything uses the same 1 through 5 scale.  
...
stu_table <- stu_table |> 
  mutate(across(6:31, cell_color))

# stu_table$`Program Evaluation` = cell_spec(stu_table$`Program Evaluation`, color = ifelse(stu_table$`Program Evaluation` > 3, "red", "blue"))
...
The following two code blocks loops through the dataframes twice to formulate all student utility ratings and all organization utility ratings. In addition, 
...
```{r gale_shapley, echo=FALSE, message=FALSE, warning=FALSE}

library(matchingR)
matching = galeShapley.marriageMarket(uS, uO)

#str(matching)
#print(matching$engagements)
...
students_matched <- stu |> 
  ...
  bind_cols(org_info) |>  
  select(name, project_name, project_goal, project_deliverables) |> 
  rename(student = name)
