...
# set your api key using Sys.setenv("NAME" = "API KEY")
 ...
 api_key = "hfJlPYTIsZeDw52H9IWjwQmH8D3cI1HpYsyV93G4", 
...
org_number <- which(surveys$name=="Matching Form_Summer 2022_corporate test", arr.ind=TRUE)
...
#write.csv(stu, "data/interim/students_summer_2022[columns].csv", row.names = FALSE)

stu <- read_csv("data/interim/students_summer_2022_manual.csv")
...
org3 <- org2 |>
  ...
  mutate(across(5:29,~gsub("1 - Not relevant", "1", .)),
         across(5:29,~gsub("5 - Extremely relevant", "5", .)),
         across(5:29, as.numeric),
         conducting_interviews = replace_na(1))
...
org_table <- org_table |> 
  rename("relevant_skills" = "What Other Relevant Skills Would Be Helpful For Your Candidate To Have I E Other Languages Spoken Coding Analytical Software Professional Skills Etc List Them Here")
...
```{r more_cleaning, layout="l-body-outset"}
...
g <- ggplot(stu5, aes(x = reorder(Skill, n), y = n, fill = n)) +
...
stu6 <- stu4 |> 
  #remove nonprofit experience
  select(-nonprofit_experience, time_commitment)


#Initial Data Manipulation
org1 <- org3[1, ] |>
  pivot_longer(5:29, names_to = "variable", values_to = "org_value")
stu1 <- stu6[1, ] |>
  pivot_longer(6:30, names_to = "variable", values_to = "student_value")
...
uO <- matrix(nrow = length(stu6$name),
