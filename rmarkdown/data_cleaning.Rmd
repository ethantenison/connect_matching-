...
stu1 <- stu[1,] |> 
  ...
  select(name, student_value)


match1 <- bind_cols(org1,stu1) |> 
  mutate(subtracted_preference = org_value - student_value,
         utility = case_when(
           subtracted_preference <= 0 ~ 1,
           subtracted_preference == 1 ~ .75,
           subtracted_preference == 2 ~ .5,
           subtracted_preference == 3 ~ .25,
           subtracted_preference == 4 ~ 0
         ),
...
output <- matrix(ncol=length(org$organization), nrow=length(org$organization))

#nested for loop to get utility scores 

u <- matrix(nrow = 3, ncol = 24)
