...
uO <- matrix(nrow = length(org$organization),
...
for (i in 1:length(org$organization)) {
  for (j in 1:length(stu$name)) {
    ...
    stu1 <- stu[j,] |>
      ...
      dplyr::select(name, student_value)
...
uS <- matrix(nrow = length(stu$name),
...
matching = galeShapley.marriageMarket(uO, uS)
```
